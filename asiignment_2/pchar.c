...
static __init int pchar_init(void){
    ...
    printk(KERN_ERR "%s : kfifo_alloc() sucesssfully created device.\n",THIS_MODULE->name);
    ...
    printk(KERN_INFO "%s : class_create() created device clas.\n",THIS_MODULE->name);
    ...
    if(ret !=0){
        printk(KERN_ERR "%s : cdev_add() failed to ad cdev in kernel db.\n",THIS_MODULE->name);
