...
MODULE_PARAM_DESC(devcnt,"Number of devices");
...
static __init int pchar_init(void) {
	...
	printk(KERN_IFNO "%s : pcahr_init() called.\n",THIS_MODULE->name);
	...
	for (i=0 ; i < devcnt; i++) {
	 ret =kfifo_alloc(&device[i].buf ,MAX,GFP_KERNEL);
	 if(ret ! = 0) {
	 	printk(KERN_ERR "%s : kfifo_alloc() failed for devices %d.\n",THIS_MODULE->name,i);

		goto kfifo_ alloc_failed;

		}
		printk(KERN_IFNO "%s : kfifo_alloc() sucesfully created %d devices.\n",THIS_MODULE->name,devcnt);
		...
		if(IS_ERR(pclass)){
			print(KERN_ERR "%s : class_created() failed.\n",THIS_MODULE->name);
			goto class_create_failed;

			}

			print(KERN_INFO "%s : class_create() created device class.\n",THIS_MODULE->name);
